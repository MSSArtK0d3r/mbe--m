...
class Expenses extends Security_Controller {
    ...
    function modal_form() {
        ...
        $get_project_id = $this->request->getGet('project_id');
        ...
        $view_data['members_dropdown'] = $this->Users_model->get_dropdown_list_with_blank_option(array("first_name", "last_name"), "-", array("user_type" => "staff"));
        $view_data['clients_dropdown'] = null;

        $can_access_clients = $this->can_access_clients();

        if ($model_info->project_id && $can_access_clients) {
        ...
        } else if ($can_access_clients) {
            $view_data['clients_dropdown'] = $this->Clients_model->get_id_and_text_dropdown(array("company_name"), array("is_lead" => 0), "-");
        }

        $view_data['projects_dropdown'] = $this->get_projects_dropdown(0, true);
        $view_data['taxes_dropdown'] = $this->Taxes_model->get_dropdown_list_with_blank_option(array("title"));

        $view_data['model_info'] = $model_info;
        $view_data['client_id'] = $client_id;
        $view_data['project_id'] = $project_id;
        $view_data['get_project_id'] = $get_project_id;
