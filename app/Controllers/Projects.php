...
class Projects extends Security_Controller {
    ...
    function all_projects($status_id = 0) {
        ...
        $rules = [
            'date' => 'regex_match[/^\d{4}(-(0[1-9]|1[0-2]))?$/]'
        ];
        ...
        //$dateyear = $this->request->getGet('year') ?? date('Y');
        
        if (!$validation->setRules($rules)->run(['date' => $date])) {
            $view_data["error"] = "Invalid date format. Please use YYYY-MM.";
            $view_data["projects"] = []; // Empty projects if error occurs
        } else {
       // if ($this->login_user->user_type === "staff") {
            $view_data["can_edit_projects"] = $this->can_edit_projects();
            $view_data["can_delete_projects"] = $this->can_delete_projects();
            $view_data["error"] = null; // No error
            $view_data["projects"] = $this->Projects_model->get_projects_my_month($date);
            $view_data["pstatus"] = $this->Projects_model->get_status_name($status_id);
        }
    ...
    private function _get_project_info_data($project_id) {
        ...
        if ($project_info) {
            $view_data['project_info'] = $project_info;
            $view_data['project_claim'] = $this->Projects_model->get_total_invoices_progress_claim_amount($project_id);
            $view_data['project_vo'] = $this->Projects_model->get_projects_total_vo($project_id);
            $view_data['project_claim_payments'] = $this->Projects_model->get_total_payments($project_id);
            $view_data['project_total_retention'] = $this->Projects_model->get_projects_total_retention($project_id);
            $view_data['tender_expenses'] = $this->Projects_model->get_total_amount_tender_expenses($project_info->tender_id);
           // $view_data['project_claim'] = '9000';
            ...
            $view_data['project_expenses_list'] = $this->Projects_model->get_expenses_list($project_id);
            $view_data['project_expenses_list_material'] = $this->Projects_model->get_expenses_list_material($project_id);
            $view_data['project_expenses_list_tools'] = $this->Projects_model->get_expenses_list_tools($project_id);
            $view_data['project_expenses_list_labour'] = $this->Projects_model->get_expenses_list_labour($project_id);
            $view_data['project_expenses_list_other'] = $this->Projects_model->get_expenses_list_other($project_id);
            $view_data['project_expenses_total'] = $this->Projects_model->get_total_amount_expenses($project_id);
    ...
    function change_status($project_id, $status_id) {
        if ($project_id && $this->can_edit_projects() && $status_id) {
            validate_numeric_value($project_id);
            validate_numeric_value($status_id);
            $status_name = $this->Projects_model->get_status_name($status_id);
            $status_data = array(
                "status_id" => $status_id,
    ...
    function get_member_suggestion_to_mention() {
        ...
        $project_members = $this->Project_members_model->get_project_members_dropdown_list($project_id, "", $can_access_client)->getResult();
        $project_members_dropdown = array();
        foreach ($project_members as $member) {
    ...
    function modal_vo(){
        ...
        return $this->template->view('projects/modal_vo', $data); 
    }
    ...
    function save_vo(){
        ...
        $vo_date = $this->request->getPost('vo_date');
        ...
        $insert = $db->table('projects_vo')->insert($data);
        
        if ($insert) {
            return redirect()->to(base_url('projects/view/'.$pid))->with('success', 'Project saved successfully!');
