            ...
            <?php if (!empty($tender_expenses)): ?>
                <?php foreach ($tender_expenses as $expense): ?>
                    <div class="d-flex p-2 justify-content-between">
                        ...
                        <div class="w40p text-black text-end">RM <?= number_format($expense->amount, 2) ?> <a href="/tender/expense_delete?id=<?=esc($expense->texp_id)?>"><i data-feather='x' class='icon-16'></i></a></div>
                    </div>
                <?php endforeach; ?>
